...
class GPUNewtonRaphson : public IPowerFlowSolver {
  ...
  public:
    ...
    /**
     * @brief Batch solve with GPU-resident admittance matrix
     * Keeps admittance matrix on GPU across all scenarios, only updates load data
     */
    BatchPowerFlowResult solve_power_flow_batch(std::vector<NetworkData> const& network_scenarios,
        ...
        auto& logger = gap::logging::global_logger;
        logger.setComponent("GPUNewtonRaphson::Batch");
        LOG_INFO(logger, "Starting batch power flow solution on GPU");
        ...
        // GPU keeps admittance matrix resident across scenarios
        // This avoids repeated host-to-device transfers
        LOG_INFO(logger, "Using GPU-resident admittance matrix for all", network_scenarios.size(),
                 "scenarios");
        ...
        for (size_t i = 0; i < network_scenarios.size(); ++i) {
            ...
            // Solve this scenario (admittance matrix transfers are minimized)
