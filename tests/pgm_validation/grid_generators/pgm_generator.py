...
class PGMGridGenerator:
    ...
    def _generate_loads(
        self, n_load: int, load_p_w_min: float, load_p_w_max: float, pf: float
    ) -> pd.DataFrame:
        """Generate symmetric load data."""
        # Generate random load powers (per phase for symmetric load)
        p_specified = self.rng.uniform(
            load_p_w_min / 3.0, load_p_w_max / 3.0, size=n_load
        )
        ...
        return pd.DataFrame(
            {
                "id": np.arange(
                    load_id_offset, load_id_offset + n_load, dtype=np.int32
                ),
                "node": np.arange(
                    1, n_load + 1, dtype=np.int32
                ),  # Loads on nodes 1 to n_load
    ...
    def _generate_load_profiles(
        ...
        scaling = self.rng.uniform(
            load_scaling_min, load_scaling_max, size=(n_step, n_load)
        )
...
if __name__ == "__main__":
    ...
    grid = generator.generate_symmetric_radial_grid(
        n_feeder=2, n_node_per_feeder=3, n_step=1
    )
