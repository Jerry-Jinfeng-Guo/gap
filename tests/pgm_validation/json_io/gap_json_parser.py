...
class PGMJSONParser:
    ...
    def parse_network(
        self, input_file: Path, update_file: Optional[Path] = None
    ) -> GAPNetworkData:
        """
    ...
    def _parse_nodes(self, nodes_json: List[Dict]) -> Dict[str, np.ndarray]:
        ...
        return {
            ...
            "id_to_idx": {
                node_id: idx for idx, node_id in enumerate(node_ids[sorted_indices])
            },
        }

    def _parse_lines(
        self, lines_json: List[Dict], nodes_data: Dict
    ) -> Dict[str, np.ndarray]:
        ...
        from_node_idx = np.array(
            [id_to_idx[node_id] for node_id in line_df["from_node"]], dtype=Index
        )
        to_node_idx = np.array(
            [id_to_idx[node_id] for node_id in line_df["to_node"]], dtype=Index
        )

        # Calculate status (both from and to must be active)
        status = (
            line_df.get("from_status", 1).values * line_df.get("to_status", 1).values
        ).astype(Index)
    ...
    def _parse_loads(
        ...
        node_idx = np.array(
            [id_to_idx[node_id] for node_id in load_df["node"]], dtype=Index
        )
    ...
    def _parse_sources(
        self, sources_json: List[Dict], nodes_data: Dict
    ) -> Dict[str, np.ndarray]:
        ...
        node_idx = np.array(
            [id_to_idx[node_id] for node_id in source_df["node"]], dtype=Index
        )

        return {
            "id": source_df["id"].values.astype(Index),
            "node_idx": node_idx,
            "status": source_df.get("status", np.ones(len(source_df))).values.astype(
                Index
            ),
            "u_ref": source_df["u_ref"].values.astype(Float),
            "sk": source_df.get("sk", np.full(len(source_df), 1e20)).values.astype(
                Float
            ),
            "rx_ratio": source_df.get(
                "rx_ratio", np.full(len(source_df), 0.1)
            ).values.astype(Float),
        }

    def _convert_lines_to_pu(
        self, lines_data: Dict, base_impedance: float
    ) -> Dict[str, np.ndarray]:
    ...
    def _convert_loads_to_pu(
        self, loads_data: Dict, base_power: float
    ) -> Dict[str, np.ndarray]:
    ...
    def _convert_sources_to_pu(self, sources_data: Dict) -> Dict[str, np.ndarray]:
        ...
        return {"u_ref": (sources_data["u_ref"]).astype(Float)}  # Already in p.u.
...
if __name__ == "__main__":
    ...
    if (test_dir / "input.json").exists():
        try:
            ...
            print(
                f"  Total load: P={np.sum(S.real)*network.base_power/1e6:.2f} MW, "
