...
class GAPJSONSerializer:
    ...
    def _serialize_node_results(self, results: GAPPowerFlowResults) -> List[Dict[str, Any]]:
    ...
    def _serialize_line_results(self, results: GAPPowerFlowResults) -> List[Dict[str, Any]]:
        ...
        for i in range(len(results.branch_ids)):
            line_results.append({
                ...
                's_from': float(np.sqrt(results.p_from_mw[i]**2 + results.q_from_mvar[i]**2)),
                'p_to': float(results.p_to_mw[i]),
                'q_to': float(results.q_to_mvar[i]),
                'i_to': float(results.i_to_a[i]),
                's_to': float(np.sqrt(results.p_to_mw[i]**2 + results.q_to_mvar[i]**2))
            })
        
        return line_results
    
    def _serialize_load_results(self, results: GAPPowerFlowResults) -> List[Dict[str, Any]]:
        ...
        for i in range(len(results.load_ids)):
            load_results.append({
                ...
                's': float(np.sqrt(results.p_load_mw[i]**2 + results.q_load_mvar[i]**2)),
                'pf': float(results.p_load_mw[i] / max(1e-12, np.sqrt(results.p_load_mw[i]**2 + results.q_load_mvar[i]**2)))
            })
        
        return load_results
    
    def _serialize_source_results(self, results: GAPPowerFlowResults) -> List[Dict[str, Any]]:
        ...
        for i in range(len(results.source_ids)):
            source_results.append({
                ...
                's': float(np.sqrt(results.p_source_mw[i]**2 + results.q_source_mvar[i]**2)),
                'pf': float(results.p_source_mw[i] / max(1e-12, np.sqrt(results.p_source_mw[i]**2 + results.q_source_mvar[i]**2))),
    ...
    def _compare_node_results(
        ...
        return {
            ...
            'rms_voltage_error_pu': float(np.sqrt(np.mean(np.array(voltage_errors)**2))),
            'max_angle_error_deg': float(max_angle_error),
            'rms_angle_error_deg': float(np.sqrt(np.mean(np.array(angle_errors)**2))),
            'voltage_within_tolerance': bool(max_voltage_error <= tolerance['voltage_pu']),
    ...
    def _evaluate_overall_match(
        ...
        all_within_tolerance = (
            node_comp.get('voltage_within_tolerance', False) and
            node_comp.get('angle_within_tolerance', False) and
            line_comp.get('p_within_tolerance', False) and
            line_comp.get('q_within_tolerance', False) and
            load_comp.get('p_within_tolerance', False) and
            load_comp.get('q_within_tolerance', False)
        )
