...
class PGMReferenceSolver:
    ...
    def generate_reference_solution(
        ...
        if validation_errors:
            raise ValueError(
                f"Input validation failed: {errors_to_string(validation_errors)}"
            )
        ...
        try:
            if update_data:
                ...
                result = model.calculate_power_flow(
                    update_data=update_data, **calc_params
                )
    ...
    def _convert_to_pgm_update(self, update_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "sym_load" in update_data:
            ...
            load_update = pgm.initialize_array(
                "update", "sym_load", (n_scenario, n_load)
            )
    ...
    def _convert_pgm_output(
        self, pgm_result: Dict[pgm.ComponentType, np.ndarray], calculation_time: float
    ) -> Dict[str, Any]:
        ...
        if pgm.ComponentType.node in pgm_result:
            output_data["node"] = self._convert_node_output(
                pgm_result[pgm.ComponentType.node]
            )

        # Convert line results
        if pgm.ComponentType.line in pgm_result:
            output_data["line"] = self._convert_line_output(
                pgm_result[pgm.ComponentType.line]
            )

        # Convert load results
        if pgm.ComponentType.sym_load in pgm_result:
            output_data["sym_load"] = self._convert_load_output(
                pgm_result[pgm.ComponentType.sym_load]
            )

        # Convert source results
        if pgm.ComponentType.source in pgm_result:
            output_data["source"] = self._convert_source_output(
                pgm_result[pgm.ComponentType.source]
            )
    ...
    def generate_test_suite_references(
        self, test_cases_dir: Path, output_dir: Path, pattern: str = "input*.json"
        ...
        for input_file in input_files:
            ...
            update_file = (
                input_file.parent / f"update_{test_name.replace('input_', '')}.json"
            )
            ...
            try:
                ...
                results[test_name] = {"error": str(e), "failed": True}
...
if __name__ == "__main__":
    ...
    if (test_dir / "input.json").exists():
        ...
        print(
            "No test input file found. Generate test data first using PGM grid generator."
        )
