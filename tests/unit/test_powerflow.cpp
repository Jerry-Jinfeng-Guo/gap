...
void test_cpu_powerflow_creation() {
    ...
    ASSERT_EQ(BackendType::CPU, pf_solver->get_backend_type());
}
...
void test_powerflow_solve_simple() {
    ...
    matrix.values = {
        Complex(10.0, -20.0), Complex(-5.0, 10.0),
        Complex(-5.0, 10.0), Complex(10.0, -20.0)
    };
...
void test_gpu_powerflow_availability() {
    ...
    if (gpu_available) {
        ...
        ASSERT_TRUE(pf_solver != nullptr);
        ASSERT_EQ(BackendType::GPU_CUDA, pf_solver->get_backend_type());
    }
